...
function play() {
    ...
    if (players[currPlayer].inJail && diceVal1 != diceVal2) {
        // show jail popup
        var jailQstn = document.createElement("div");
        $(jailQstn)
            .addClass("logItem")
            .html("Pay " + rupeeSym + "100 OR wait till you roll a double?");
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire("", "Paid " + rupeeSym + "100 to get out of Jail", "success");
    ...
    if (diceVal1 == diceVal2 && !players[currPlayer].inJail) {
        ...
        if (consecutiveDoubles < 3) diceRolled = false;
        else {
...
function moveBackWithoutGO(endPos) {
    ...
    var coinMoveAnim = setInterval(function () {
        ...
        if (players[currPlayer].position > 30 && endPos < 30) players[currPlayer].position = 30;
        else if (players[currPlayer].position > 20 && endPos < 20) players[currPlayer].position = 20;
        else if (players[currPlayer].position > 10 && endPos < 10) players[currPlayer].position = 10;
        else players[currPlayer].position = endPos;
...
function moveAheadWithGO(endPos) {
    ...
    var coinMoveAnim = setInterval(function () {
        ...
        if (endPos > players[currPlayer].position) {
            ...
            if (players[currPlayer].position < 10) players[currPlayer].position = 10;
            else if (players[currPlayer].position < 20) players[currPlayer].position = 20;
            else if (players[currPlayer].position < 30) players[currPlayer].position = 30;
            else if (players[currPlayer].position < 40) players[currPlayer].position = 0;
        }
...
function checkCell() {
    ...
    if (cellRent != undefined) {
        if (cellRent > 0) {
            ...
            if (players[currPlayer].money < cell.price) $("#btnBuyProperty").prop("disabled", true);
            else $("#btnBuyProperty").prop("disabled", false);
        ...
        if (cell.cellName == "Chest") {
            ...
            if (cell.position == 30) moveBackWithoutGO(10);
        }
    }
    refreshGameUI();
    return;
...
function changePlayer() {
    ...
    if (players[currPlayer].bankrupt) changePlayer();
}
...
$(document).ready(function () {
    ...
    $("#btnBuyProperty").click(function () {
        ...
        Swal.fire("", "Bought " + cell.cellName + " for " + rupeeSym + " " + cell.price, "success");
