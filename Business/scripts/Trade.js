...
function refreshTradeCards(left) {
    if (left) {
        ...
        while (i < players[leftPlayer].properties.length) {
            ...
            if (board[cardNumber].isTradeble()) addTradeCardImage(cardNumber, left);
            i += 1;
        }
    } else {
...
$(document).ready(function () {
    $(document).on("click", ".imageLeftPropertyTrade", function () {
        if ($(this).hasClass("tradeImageSelected")) $(this).removeClass("tradeImageSelected");
        else $(this).addClass("tradeImageSelected");
    });
    ...
    $("#confirmTrade").click(function () {
        if (
            $("input[name='tradeLeftPlayerSelector']:checked").val() ==
            $("input[name='tradeRightPlayerSelector']:checked").val()
        ) {
            Swal.fire("Trade Fail!", "Same player selected", "error");
        } else {
            Swal.fire({
                title: "Are you sure?",
                ...
                if (result.isConfirmed) {
                    if (performTrade()) {
                        Swal.fire("Trade complete!", "", "success");
                        refreshGameUI();
                    } else {
                        Swal.fire("Trade Fail!", "", "error");
                    }
                    $("#TradeTray").animate({ right: "-650px" });
                }
            });
