        ...
        switch (this.number) {
            ...
            case 9: {
                var log = new Log(
                    currPlayer,
                    -1,
                    this.amount * players[currPlayer].properties.length,
                    this.message + " " + rupeeSym + this.amount.toString(),
                );
                log.generateLogDiv();
                log.prependLogDiv();
                log.performTransaction();
        ...
        switch (this.number) {
            case 6: {
                var log = new Log(
                    currPlayer,
                    -1,
                    this.amount * players[currPlayer].houseCount,
                    this.message + this.amount.toString(),
                );
                log.generateLogDiv();
                log.prependLogDiv();
                log.performTransaction();
            ...
            case 12: {
                ...
                if (players[currPlayer].position < 0) players[currPlayer].position = 40 - players[currPlayer].position;
