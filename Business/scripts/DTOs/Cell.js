class Cell {
            ...
            if (this.owner > -1) {
                if (this.isMortgaged) {
                    ...
                    if (this.position == 5 || this.position == 15 || this.position == 25 || this.position == 35) {
                        ...
                        var i = 0;
                        while (i < 4) {
                            if (players[this.owner].properties.includes(stations[i])) rent = rent + rent;
                            i += 1;
                        }
                    ...
                    if (this.position == 12 || this.position == 28) {
                        if (
                            players[this.owner].properties.includes(12) &&
                            players[this.owner].properties.includes(28)
                        ) {
    ...
    isBuildable() {
        ...
        if (this.owner < 0) return false;

        if (!players[this.owner].cityGroups.includes(this.colorGroup)) return false;
        ...
        while (i < propertyColorGroups[this.colorGroup].length) {
            ...
            if (board[cardNumber].isMortgaged) return false;

            if (board[cardNumber].houses < this.houses) return false;
    ...
    isMortgageable() {
        if (!this.isCity && !this.isUtility) return false;
        if (this.isMortgaged) return false;
        if (this.hotel) return false;
        if (this.houses > 0) return false;

        if (!players[this.owner].cityGroups.includes(this.colorGroup)) return true;

        var i = 0;
        while (i < propertyColorGroups[this.colorGroup].length) {
            var cardNmbr = propertyColorGroups[this.colorGroup][i];
            if (board[cardNmbr].houses > 0) return false;
            i += 1;
        }
        return true;
    }
